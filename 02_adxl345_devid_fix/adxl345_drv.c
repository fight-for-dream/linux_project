			    ...
			    size_t count, loff_t *ppos)
{
	...
	/*
	 * SPI controllers normally cannot detect that a slave was unplugged.
	 * Verify ADXL345's fixed device ID before every sample so a floating MISO
	 * line returning 0x00 or 0xff is reported to user space as a fault.
	 */
	ret = adxl345_read_reg(sensor, ADXL345_REG_DEVID, &device_id);
	if (!ret && device_id != ADXL345_DEVID)
		ret = -ENODEV;
	if (!ret)
