...
static void set_mqtt_connection_state(int online, void *user_data)
{
	struct app_context *context = user_data;

	pthread_mutex_lock(&context->data_mutex);
	context->mqtt_online = online;
	pthread_mutex_unlock(&context->data_mutex);
}
...
int main(int argc, char **argv)
{
	...
	mqtt_context.set_connection_state = set_mqtt_connection_state;
	...
	while (!stop_requested)
	{
		...
		if ((int32_t)(now_ms - next_ui_update) >= 0)
		{
			...
			mqtt_online = context.mqtt_online;
			pthread_mutex_unlock(&context.data_mutex);
