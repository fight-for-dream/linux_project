...
#define MQTT_STATUS_TOPIC	"device/imx6ull/status"
...
{
	struct mqtt_publish_context *context = reconnect_data;

	(void)client;
	if (context && context->set_connection_state)
		context->set_connection_state(0, context->user_data);
}

static int publish_connection_status(mqtt_client_t *client,
				     const char *status)
{
	...
	return mqtt_publish(client, MQTT_STATUS_TOPIC, &message);
}
...
void *mqtt_publish_thread(void *argument)
{
	...
	mqtt_set_reconnect_handler(client, reconnect_handler);
	mqtt_set_will_options(client, MQTT_STATUS_TOPIC, QOS1, 1, "offline");
	if (context->set_connection_state)
		context->set_connection_state(0, context->user_data);
	...
	while (!*context->stop_requested)
	{
		...
		if (!ret)
		{
			...
			ret = publish_connection_status(client, "online");
			if (ret)
				fprintf(stderr, "MQTT online status failed: %d\n", ret);
			else
				online_reported = 1;
			if (context->set_connection_state)
				context->set_connection_state(1,
						      context->user_data);
			break;
		}
	...
	while (!*context->stop_requested && connected_once)
	{
		...
		if (ret)
		{
			...
			online_reported = 0;
			if (context->set_connection_state)
				context->set_connection_state(0,
						      context->user_data);
		}
		else
		{
	...
	if (connected_once)
	{
		publish_connection_status(client, "offline");
		if (context->set_connection_state)
			context->set_connection_state(0, context->user_data);
