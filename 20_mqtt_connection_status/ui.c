...
static lv_obj_t *mqtt_state_label;
...
void ui_create(const struct ui_operations *operations, void *user_data)
{
	...
	lv_obj_set_pos(subtitle, 650, 29);

	mqtt_state_label = lv_label_create(screen);
	lv_label_set_text(mqtt_state_label, "MQTT OFFLINE");
	lv_obj_set_style_text_font(mqtt_state_label,
				   &lv_font_montserrat_14, 0);
	lv_obj_set_style_text_color(mqtt_state_label,
				    lv_palette_main(LV_PALETTE_RED), 0);
	lv_obj_set_pos(mqtt_state_label, 895, 29);
	...
	motor_value = create_value_label(motor_card, "CLOSED", 20);
...
void ui_set_temperature(float value, int alarm, int valid, int fault)
{
	...
	if (fault)
	{
		lv_label_set_text(temperature_value, "ERROR");
		lv_label_set_text(temperature_state, "SENSOR ERROR");
		lv_obj_set_style_text_color(temperature_state,
					    lv_palette_main(LV_PALETTE_RED), 0);
		return;
	}
	if (!valid)
...
void ui_set_humidity(float value, int alarm, int valid, int fault)
{
	...
	if (fault)
	{
		lv_label_set_text(humidity_value, "ERROR");
		lv_label_set_text(humidity_state, "SENSOR ERROR");
		lv_obj_set_style_text_color(humidity_state,
					    lv_palette_main(LV_PALETTE_RED), 0);
		return;
	}
	if (!valid)
...
void ui_set_acceleration(float x, float y, float z, int valid, int fault)
{
	...
	if (fault)
	{
		lv_label_set_text(acceleration_value, "SENSOR ERROR");
		lv_obj_set_style_text_color(acceleration_value,
					    lv_palette_main(LV_PALETTE_RED), 0);
		return;
	}
	if (!valid)
	...
	lv_obj_set_style_text_color(acceleration_value, lv_color_hex(0x0f172a), 0);
...
void ui_set_vibration(float value, int alarm, int valid, int fault)
{
	...
	if (fault)
	{
		lv_label_set_text(vibration_value, "ERROR");
		lv_label_set_text(vibration_state, "SENSOR ERROR");
		lv_obj_set_style_text_color(vibration_state,
					    lv_palette_main(LV_PALETTE_RED), 0);
		return;
	}
	if (!valid)
...
void ui_set_mqtt_state(int online)
{
	...
	if (online)
	{
		lv_label_set_text(mqtt_state_label, "MQTT ONLINE");
		lv_obj_set_style_text_color(mqtt_state_label,
